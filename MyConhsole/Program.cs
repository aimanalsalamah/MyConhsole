...
switch (Console.ReadLine())
{
    case "test":
        ...
        break;
    case "data":
        foreach (var item in Tools.JsonFunctions.DeserializeJson<List<Tools.Model.User>>("Data"))
        {
            Console.WriteLine($"name:{item.name}, Age:{item.Age}, Mobile:{item.Mobile}");
        }
        break;
    case "customer":
        foreach (var item in Tools.JsonFunctions.DeserializeJson<List<Tools.Model.Customer>>("Customer"))
        {
